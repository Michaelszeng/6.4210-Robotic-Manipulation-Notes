...
The paper considers 2 cost functions:
...
- if we assume robot starts from rest ("free initial cbdutuibs"):
$$ x(q(t_0)) = x_{ref}(t_0) $$
$$ \dot{q}(t_0) = 0 $$

- if we assume the robot performs cyclical/repetitive task (i.e. in industrial settings) ("cyckucak initial conditions"):
$$ q(t_0) = q(t_{fin}) $$
...
4. Repeat 1-3 until a stop criteion is fulfilled.

We go into more detail below.

#### Generating Candidate Solutions

To generate candidate solutions, use inverse kinematics but with a weighted pseudo-inverse of Jacobian defined like so:
...
We can then calculate $\dot{q}$ that result in the desired spatial velocity specified in our desired trajectory:
$$ \dot{q} = J_w^+ v $$

To generate our candidate solutions (step 1 above), we generate a set of $n_{candidates}$ random weight matrices $W$. We generally pick $n_{candidates} = n_{path points}^2 * n_{joints}^2$ to ensure we get a good optimum. We use all these weight matrices to create $n_{candidates}$ different Jacobians to calculate $n_{candidates}$ different initial joint positions $q_{random}$ to satisfy the initial spatial position of the end-effector. We then extrapolate each $q_{random}$ into sets of joint positions $\{q_{random}\}$ for the entire trajectory.
...
which basically sums the distance from every joint position to its respective joint mechanical limits ($q_{iM}$ is the max position, $q_{im}$ is the min position, $\bar{q}_i$ is the middle between max and min, and there are $n$ joints), at every time step over the trajectory.

We then pick the best $n_{runs}$ solutions of $\{q_{random}\}$ ($n_{runs}$ is arbitrarily set to 24). We then enforce joint limit constraints for all $q$. 
