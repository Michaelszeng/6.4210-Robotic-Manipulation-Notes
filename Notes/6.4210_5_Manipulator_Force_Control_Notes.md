...
Now, let's plug that into the manipulators dynamics/net force equation. First, recall the manipulator's net force equation:

$$ M(q)\ddot{q} + C(q,\dot{q})\dot{q} = \tau_g(q) + u + \tau_{ext} $$

where the mass*acceleration of the manipulator (with the Coriolis term) equals all the torques from gravity, motor commands, and contact forces on each joint.
...
### Controlling Robot force in Spatial Frame
...
Before going further, we need to understand a critical translation from spatial-force to joint torqus: 
...
We can achieve such dynamics (by plugging in the control force policy into the net force equation):
