...
m & 0 \\
0 & m
\end{bmatrix} \dot{v} = \begin{bmatrix} \ddot{x} \\ \ddot{z} \end{bmatrix} = \begin{bmatrix} 0 \\ -mg \end{bmatrix} + \begin{bmatrix} u_x \\ u_z \end{bmatrix} + f^{R_c}$$
...
Especially if manipulator affected by gravity, causes steady-state error. This can be seen by showing the net force equation for the manipulator (assuming no contact forces):
...
Slight mod to PID method that also takes into account gravity.
...
The steady-state error due to gravity is gone, but the performance is still mediocre:
...
Modify JointStiffnessController by adding $\ddot{q}_d(t)$ feedforward term. Basically, whenever we want the manipulator to accelerate, this adds extra torque to achieve that acceleration.

$$ u = mg + m(\ddot{q}_d(t) + k_p(q_d - q) + k_d(\dot{q_d} - \dot{q}))$$

However, there's aother key difference: the `JointStiffnessController` wrote the feedback law in terms of forces; `InverseDynamicsController` writes it in accelerations???
...
Consider the simple case when the robot is in contact/pushing an object: acceleration of manipulator is $\approx 0$. Then, net force becomes: 
...
This will calculate the desired contact force $f^{B_C}_{finger,C_x}$, satisfying the constraints (so the box doesn't slip or move at point $A$), to rotate the box.
...
$$ f^{R_c} = m \begin{bmatrix} \ddot{x} \\ \ddot{z} \end{bmatrix} + b \begin{bmatrix} \dot{x} \\ \dot{b} \end{bmatrix} + k \begin{bmatrix} x-x_d \\ z-z_d \end{bmatrix} $$

This depends solely on the current velocity, acceleration, and position error of the manipulator. The equation is the differential equation for a linearly damped spring sytem. So, what this controller bascally does, is put the manipulator on a virtual spring-damper connected to a virtual setpoint $(x_d, z_d)$. Wherever you place $(x_d, z_d)$, the manipulator will follow in a spingy/damped motion.
...
To control the torque inputs into the manipulator, we can use a `JointStiffnessController` as described above (PD controller that takes into account gravity). This will go from the desired contact force to a motor comand.

More general form of the equation:

$$ m \space {}^W\dot{v}^R + K_d \space {}^Wv^R + K_p (^Wp^R -{}^Wp^{R_d}) = f^R $$

where $^Wp^R$ is the position of the manipulator in world frame.
...
## Control of a Real Manipulator

All examples above use a point-mass manipulator for simplicity. Now, we move to a real 7-DoF manipulator.

First, recall the manipulator's net force equation:

$$ M(q)\ddot{q} + C(q,\dot{q})\dot{q} = \tau_g(q) + u + \sum_i J_i^T(q) F^{c_i} $$

where the mass*acceleration of the manipulator (with the Coriolis term) equals all the forces from gravity, motor commands, and contact forces.

Second, we define a frame $E$, at the point of contact between the end-effector and environment (assume all contact forces happen at $E$, and the robot doesn't contact the environment anywhere else). Therefore, we can define the position, velocity, and acceleration of the end effector like so (for now, we ignore rotation):
...
Our goal is to express all forces on the manipulator, including the forces from the joints themselves, in the frame $E$, so we rewrite the net force equation above:
