...
[0, 0.1, 0] is the point in gripper frame in the middle between the 2 fingers. [0, 0.1, 0.02] and [0, 0.1, -0.02] are points equidistant from the 2 fingers, but shifted "left"/"right". We're saying that these 2 points in gripper frame must coincide with the center axis of the cylinder.
...
Visualizing optimizations: https://manipulation.csail.mit.edu/data/shelf_ik_prog.html
...
To solve the entire trajectory, instead of performing an optimization for every $q$ in the trajectory, we optimize the whole trajectory at once (with the cost being time taken):
...
$\alpha$ here are parameters that define the trajectory. We define $\alpha$ as the control points of a B-spline trajectory (similar to bezier curves). Why a B-spline?
...
- The B-spline is guaranteed to be within the area bounded by the control points
