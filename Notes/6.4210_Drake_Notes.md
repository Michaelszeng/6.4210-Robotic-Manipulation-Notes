...
1. Procedural Code running in separate thread that passes messages to a modul that runs n the main robot thread
2. Task-level policies (i.e. FSMs) or "behavior trees" that get evaluated in the robot loop
3. Task-level planners chain together task-level behaviors to achieve long-term behavior (i.e. trajectory planners)???
